...
export default function ResourcesWebinars() {
  ...
  useEffect(() => {
    ...
    if (curated && curated.length) {
      ...
      getResources({ category: 'WEBINAR' }).then(setItems).catch(() => {});
    }
  }, [getResources]);
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Webinaires</h1>
        <p className="text-sm text-muted-foreground">Tous les contenus de type webinaire</p>
      </div>
      ...
      <Card className="bg-card/60 backdrop-blur">
        <CardContent className="pt-6">
          <div className="relative max-w-xl">
            ...
            <Input placeholder="Rechercher un webinaire…" className="pl-8" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
        </CardContent>
      </Card>
